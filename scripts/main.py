...
def determine_best_split():
    cv_results, test_set_results = metrics_calc.train_with_cv(data=dataframe, ratings_in_subsets=[50000, 100000, 150000, 200000],test_set_percentages=[0.3, 0.5, 0.8], neighbors_counts=[100],k_fold=3)
...
def train_model(best_split):
    cv_results, test_set_results = metrics_calc.train_with_cv(data=dataframe, ratings_in_subsets=[all_ratings_count],test_set_percentages=[best_split], neighbors_counts=[25,50,75,100,125,150,175,200,6000],k_fold=3)
    cv_results_all.append(cv_results, ignore_index = True)                       
    test_set_results_all.append(test_set_results, ignore_index = True)
